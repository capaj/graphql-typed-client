...
export const GqlClient = (
  toRequestOptions: RequestOptionsFn,
  subscriptionClientOptions?: SubscriptionClientOptions
) => {
  const subscription = subscriptionClientOptions
    ? subscriptionClient(subscriptionClientOptions)
    : () => NEVER

  return {
    query: (q: QUERY_REQUEST_T): Promise<Response<QUERY_T>> =>
      request(toRequestOptions(toGql('query', q))),
    mutation: (q: MUTATION_REQUEST_T): Promise<Response<MUTATION_T>> =>
      request(toRequestOptions(toGql('mutation', q))),
    subscription: (
      q: SUBSCRIPTION_REQUEST_T
    ): Observable<Response<SUBSCRIPTION_T>> =>
...
export const toMutationGql = (q: MUTATION_REQUEST_T): GQL =>
  toGql('mutation', q)

export const toSubscriptionGql = (q: SUBSCRIPTION_REQUEST_T): GQL =>
  toGql('subscription', q)
